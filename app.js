...
document.getElementById('flipBtn').addEventListener('click', function(){
  qs('front').hidden=!qs('front').hidden;
  qs('back').hidden=!qs('back').hidden;
});
document.getElementById('nextBtn').addEventListener('click', function(){index=(index+1)%shuffled.length;renderCard();});
document.getElementById('prevBtn').addEventListener('click', function(){index=(index-1+shuffled.length)%shuffled.length;renderCard();});
document.getElementById('shuffleBtn').addEventListener('click', function(){shuffled=randShuffle([...decks[activeDeck]]);index=0;renderCard();});
document.getElementById('backBtn').addEventListener('click', function(){show('deckPicker');hide('studyArea');});
...
document.getElementById('cancelEditBtn').addEventListener('click', () => {show('deckPicker');hide('editor');});
